...
public class TeacherCourseQualificationLogic implements TeacherCourseQualificationService {
    ...
    getCourseLibraryAndTeacherCourseQualificationList(@NotNull List<CourseLibraryDO> courseLibraryDOList) {
        // 创建返回结果列表
        ...
        for(CourseLibraryDO courseLibraryDO : courseLibraryDOList) {
            // 根据课程库UUID获取教师课程资格信息
            TeacherCourseQualificationDO teacherCourseQualificationDO =
                    teacherCourseQualificationDAO.getTeacherCourseQualificationByCourseLibraryUuid(
                            courseLibraryDO.getCourseLibraryUuid());
            // 检查是否已分配教师，未分配则抛出异常
            if (teacherCourseQualificationDO == null) {
                throw new BusinessException("此" + courseLibraryDO.getName() + "课程没有分配老师教学", ErrorCode.BODY_ERROR);
            }
            // 创建课程库和教师资格的关联DTO对象
            ...
            courseLibraryAndTeacherCourseQualificationListDto1.setCourseLibraryDTO(BeanUtil.toBean(
                    courseLibraryDO, CourseLibraryDTO.class));
            // 创建教师课程资格DTO列表
