...
public class PermissionAspect {
    ...
    /**
     * 在方法执行前检查请求的角色。
     * 该方法作为切面逻辑，用于验证调用者是否具备执行特定方法所需的角色。
     *
     * <p>角色检查的流程如下：</p>
     * <ol>
     ...
     *     <li>获取当前请求的用户信息。</li>
     *     <li>验证用户的角色中是否包含所需角色。</li>
     *     <li>若最终未找到匹配角色，则抛出权限拒绝异常 {@link UserAuthenticationException}。</li>
     * </ol>
     *
     * <p><strong>注意事项：</strong></p>
     * <ul>
     *     <li>确保调用该方法的前置条件是请求上下文已建立，且 {@link RequestRole} 注解已正确应用在目标方法上。</li>
     *     <li>方法内部实现了详细的角色检查逻辑，包括直接角色和继承角色的检查。</li>
     * </ul>
     *
     * @param joinPoint 连接点对象，包含正在执行的方法信息。
     *                  通过此参数可以获取到注解 {@link RequestRole} 配置的角色标识符。
     * @throws UserAuthenticationException 如果用户未登录或没有足够角色，抛出此异常。
     */
